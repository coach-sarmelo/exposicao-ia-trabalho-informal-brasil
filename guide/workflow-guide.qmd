...
8. **Progressive disclosure.** Start with CLAUDE.md + 2--3 rules. Add more as your workflow matures. Newcomers should not face all 33 rules on day one.
...
| `/compress-session` | `.claude/skills/compress-session/` | Distil the current session into a structured note before auto-compaction (v1.9.0) |
...
| `/diagnose` | `.claude/skills/diagnose/` | Root-cause failing or wrong empirical results with a disciplined reproduce → minimise → hypothesise → instrument → fix loop (v2.0) |
...
| `/submission-disclosures` | `.claude/skills/submission-disclosures/` | Generate submission-time disclosure block (AI-use disclosure, CRediT author roles, COI, data availability) (v2.1.0) |
...
| DiD Conventions | `did-conventions.md` | `scripts/**`, `explorations/**` — Staggered DiD / event-study conventions (Callaway–Sant'Anna, HonestDiD, DR estimators) (v2.0) |
...
| Inference Robustness | `inference-robustness.md` | `scripts/**`, `explorations/**` — Standard error clustering, wild bootstrap, and spatial inference standards (v2.0) |
...
| Python Pipeline Conventions | `python-pipeline-conventions.md` | `data/scripts/**`, `data/tests/**` — Conventions for Python dual-engine pipeline, authoritative outputs in `data/output/`, and pytest suites |
