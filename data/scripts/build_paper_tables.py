...
OUT = os.path.join(repo_root, 'paper', 'tables')
...
def main():
    ...
    econ = load(os.path.join(data_dir, 'output', 'econometrics.json'))['specifications']
    panel = load(os.path.join(data_dir, 'output', 'regional_panel.json'))['data']
    scores = {k: v for k, v in load(os.path.join(data_dir, 'output', 'scores.json')).items()}
    national_raw = load(os.path.join(data_dir, 'output', 'cod_subgroups.json'))
    ...
    with open(os.path.join(data_dir, 'output', 'individual_microdata.csv'),
    ...
    for o in top:
        ...
        name_display = SHORT_OCC_NAMES.get(code, truncate(o['name'], 52))
        rows.append(
            f"{tex_escape(name_display)} & {fmt(o['total_workers'] / 1e6, 1)} & "
    ...
    rob = load(os.path.join(data_dir, 'output', 'robustness.json'))
    specs = [s1,
