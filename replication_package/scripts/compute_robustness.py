...
def oster_bound(res_uncon, res_con, r_max):
    ...
    # Oster (2019), as in psacalc/robomit: extrapolate from the CONTROLLED R2.
    # delta solves beta* = 0; beta_star is beta*(delta=1).
    denom = (b1 - b2) * (r_max - r2c)
    ...
    bstar = b2 - (b1 - b2) * (r_max - r2c) / (r2c - r2u)
    base.update({
