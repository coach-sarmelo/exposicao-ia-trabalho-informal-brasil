...
[LEARN:workflow] **Always delegate to specialized subagents and invoke dedicated skills rather than running monolithic audits in the primary agent context.** When user asks for an empirical audit, manuscript review, or claim verification, follow the skill protocol (`/audit-reproducibility`, `/verify-claims`, `/review-paper`) and spawn dedicated subagents (`claim-verifier` in forked context, `methods-referee`, `domain-referee`). Monolithic execution in the parent context bypasses the adversarial verification guarantees.

[LEARN:structure] **In Antigravity / AGY architecture, skills belong directly in `.agents/skills/<name>/SKILL.md`.** Redundant plugin nesting (`.agents/plugins/academic-workflow/`) causes duplication of rules, hooks, and references, and creates broken path assumptions. Keep `.agents/` clean: `agents/`, `hooks/`, `references/`, `rules/`, `scripts/`, `skills/`.

[LEARN:econometrics] **Oster (2019) proportional selection bounding formula must measure remaining variation relative to the controlled model ($R_{\max} - \tilde{R}$ where $\tilde{R} = R_{S2}$), not the baseline model.** In Oster (2019, JBES), $\delta = \frac{\tilde{\beta}(\tilde{R} - \mathring{R})}{(\mathring{\beta} - \tilde{\beta})(R_{\max} - \tilde{R})}$. Measuring distance from baseline $R^2$ ($R_{\max} - \mathring{R}$) overstates unexplained variation and distorts $\delta$ and $\beta^*$ bounds.
