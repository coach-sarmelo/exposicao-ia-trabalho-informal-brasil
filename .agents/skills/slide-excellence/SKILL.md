...
Before spawning the substance-review agent on a `.tex` file, verify `.agents/plugins/academic-workflow/skills/domain-reviewer.md` has been customized for this project. The ship-state domain-reviewer is a **template** — running it unmodified produces generic "are assumptions stated?" feedback, not real domain review.
...
- The five lenses are identical to the shipped template's wording (diff against `.agents/plugins/academic-workflow/skills/domain-reviewer.md` on the v1.3.0 tag — if zero lines changed, it's still template).
...
stated?") rather than field-specific review. Options:
  ...
  1. Customize .agents/plugins/academic-workflow/skills/domain-reviewer.md — replace the 5 lenses
