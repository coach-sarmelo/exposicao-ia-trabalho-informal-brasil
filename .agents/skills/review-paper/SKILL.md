...
- `--peer <JOURNAL>` — simulated peer review pipeline calibrated to `<JOURNAL>` (see `.agents/plugins/academic-workflow/references/journal-profiles.md` for available short names).
...
Simulated editorial pipeline: **editor desk review → referee selection → 2 blind referees with different dispositions → editorial synthesis**. Calibrated to a target journal from `.agents/plugins/academic-workflow/references/journal-profiles.md`. Use when: pre-submission dress rehearsal, choosing between target journals, R&R planning.
...
- `.agents/plugins/academic-workflow/skills/editor.md` — editor (desk review, referee selection, synthesis).
- `.agents/plugins/academic-workflow/skills/domain-referee.md` — substance referee.
- `.agents/plugins/academic-workflow/skills/methods-referee.md` — methodology referee (paper-type-aware).
...
6b. **Cross-artifact integration.** Unless `$ARGUMENTS` contains `--no-cross-artifact`, and if the manuscript references analysis scripts (detected via `\input{scripts/...}`, `%% source:` comments, or matching `scripts/R/_outputs/` filenames), auto-invoke:
   ...
   Merge critical cross-artifact findings (code bug invalidates paper claim, reproducibility FAIL) into a new "Cross-Artifact Findings" section at the top of the paper review report. See [`.agents/plugins/academic-workflow/rules/cross-artifact-review.md`](../../rules/cross-artifact-review.md) for the full protocol.
...
**Target journal:** [JOURNAL_SHORT] → [full name from `.agents/plugins/academic-workflow/references/journal-profiles.md`]
**Journal profile loaded:** [yes/no; resolved from `.agents/plugins/academic-workflow/references/journal-profiles.md`; key adjustments: e.g., "Identification 35 → 40"]
...
If the manuscript path doesn't exist, the target journal isn't in `.agents/plugins/academic-workflow/references/journal-profiles.md`, or a cross-artifact script is missing, stop and surface the issue before proceeding.

### Phase 1: Editor desk review

Spawn forked subagent `editor` with the manuscript path and `--peer <JOURNAL>` context. Editor:
- Reads journal profile from `.agents/plugins/academic-workflow/references/journal-profiles.md` → states "Calibrated to: [journal]".
...
- [`.agents/plugins/academic-workflow/skills/audit-reproducibility/SKILL.md`](../audit-reproducibility/SKILL.md) — numeric-claim verification (auto-invoked on referenced scripts).
- [`.agents/plugins/academic-workflow/skills/replication-package/SKILL.md`](../replication-package/SKILL.md) — assemble the AEA DCAS deposit once the paper passes review.
- [`.agents/plugins/academic-workflow/skills/capture-environment/SKILL.md`](../capture-environment/SKILL.md) · [`.agents/plugins/academic-workflow/skills/disclosure-check/SKILL.md`](../disclosure-check/SKILL.md) — environment capture + restricted-data screening for the deposit.
- [`.agents/plugins/academic-workflow/skills/seven-pass-review/SKILL.md`](../seven-pass-review/SKILL.md) — heavier 7-lens pass for submission-ready drafts.
