...
- `.agents/plugins/academic-workflow/skills/interview-me/SKILL.md` — produces the spec this skill consumes via `--input`.
- `.agents/plugins/academic-workflow/skills/power-analysis/SKILL.md` — supplies the MDE / required-N + power paragraph for the sample-plan field (RCTs).
- `.agents/plugins/academic-workflow/skills/verify-claims/SKILL.md` — Phase 5 invokes this for citation post-flight.
- `.agents/plugins/academic-workflow/references/discipline-cards.md` — field defaults that drive `--style` selection.
- `.agents/plugins/academic-workflow/rules/replication-protocol.md` — preregistration is the *forward* commitment; replication-protocol is the *backward* contract.
