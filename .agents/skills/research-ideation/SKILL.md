...
1. **Understand the input.** Read `$ARGUMENTS` and any referenced files. Check `master_supporting_docs/` for related papers. Check `.agents/plugins/academic-workflow/rules/` for domain conventions.
...
3. **Tag each RQ with a likely paper type** (drawn from `methods-referee.md`):
   ...
   Use `.agents/plugins/academic-workflow/references/discipline-cards.md` to bias the distribution by field (econ vs poli-sci default frequencies differ — e.g., poli-sci skews more toward `survey-experiment` and `formal-theory` than econ does).
...
Before returning the ideation report, run the Post-Flight Verification protocol from [`.agents/plugins/academic-workflow/rules/post-flight-verification.md`](../../rules/post-flight-verification.md). Research ideation is hallucination-prone in three specific ways:
