...
- You want a new skill that is consistent with the 40+ siblings in `.agents/plugins/academic-workflow/skills/` — same sections, same cross-reference style, same gate-passing frontmatter.
...
1. Take the kebab-case name from `$0` (or ask). Reject non-kebab-case, names that collide with an existing `.agents/plugins/academic-workflow/skills/<name>/`, or names that shadow a built-in (`commit`, `learn`, …) — `ls .agents/plugins/academic-workflow/skills/` and stop if taken.
...
3. Skim 2-3 sibling skills near the intended domain (e.g. `Glob .agents/plugins/academic-workflow/skills/*/SKILL.md`, then `Read` the closest matches) so the new skill borrows real conventions, not invented ones.
...
Write `.agents/plugins/academic-workflow/skills/<name>/SKILL.md` from the template, with these gold-standard sections:
...
`check-skill-integrity.py` enforces two parities this phase must satisfy (`.agents/plugins/academic-workflow/scripts/` hosts the gate runners; `scripts/check-skill-integrity.py` is the checker):
...
The skill is NOT discoverable to a reader until it is listed. `check-surface-sync.sh` runs a **table-row gate**: the `<!-- surface-sync-table: skills -->` tables in `README.md` and `AGENTS.md / GEMINI.md` must have exactly one data row per skill on disk. Adding a skill without a row fails the gate.

REMIND the user to:

1. Add a row to the **AGENTS.md / GEMINI.md** "Skills Quick Reference" table: `` | `/<name> [args]` | <what it does> | ``.
...
- A new file at `.agents/plugins/academic-workflow/skills/<name>/SKILL.md`.
...
- [`.agents/plugins/academic-workflow/skills/learn/SKILL.md`](../learn/SKILL.md) — capture a session discovery (the lighter sibling); `--from-learn` upgrades its output.
- [`.agents/plugins/academic-workflow/skills/coauthor-brief/SKILL.md`](../coauthor-brief/SKILL.md) — a gold-standard skill to imitate (interview → write → flags → exit-behavior shape).
- [`.agents/plugins/academic-workflow/rules/orchestrator-protocol.md`](../../rules/orchestrator-protocol.md) — why the interview collects all interactivity *before* writing.
- `.agents/plugins/academic-workflow/scripts/` and `scripts/check-skill-integrity.py` / `scripts/check-surface-sync.sh` — the gates this skill is built to pass on the first try.

## What this skill does NOT do

- **Capture a session discovery** — that is [`/learn`](../learn/SKILL.md). This skill designs an interface; `/learn` records a finding.
- **Edit the README / AGENTS.md / GEMINI.md surface tables for you.** It *prints* the two rows and reminds you; registering them (and re-running `./scripts/check-surface-sync.sh`) is a deliberate human step so the surface gate is never silently satisfied.
- **Write agents, rules, or hooks.** It scaffolds a skill only; an agent goes in `.agents/plugins/academic-workflow/skills/`, a rule in `.agents/plugins/academic-workflow/rules/`.
