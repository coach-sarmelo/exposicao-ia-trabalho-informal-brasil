...
Inputs to read:
- `.agents/plugins/academic-workflow/rules/knowledge-base-template.md` — notation registry, narrative arc, applications
- `.agents/plugins/academic-workflow/rules/content-invariants.md` — INV-1..INV-8 govern slide content
...
**First-lecture fallback (fresh fork, empty knowledge base).** If `.agents/plugins/academic-workflow/rules/knowledge-base-template.md` still has unfilled placeholder tables (no notation registry entries, no applications, no prior lectures in `Slides/`), do NOT halt waiting for it. Instead:
...
- [`.agents/plugins/academic-workflow/skills/translate-to-quarto/SKILL.md`](../translate-to-quarto/SKILL.md) — port the finished Beamer deck to a Quarto RevealJS mirror.
- [`.agents/plugins/academic-workflow/skills/qa-quarto/SKILL.md`](../qa-quarto/SKILL.md) — adversarial Beamer↔Quarto parity (loop-until-dry).
- [`.agents/plugins/academic-workflow/skills/deploy/SKILL.md`](../deploy/SKILL.md) — render + publish the lecture to GitHub Pages.
- [`.agents/plugins/academic-workflow/skills/scaffold-exercises/SKILL.md`](../scaffold-exercises/SKILL.md) — problem sets + solutions to accompany the lecture.
