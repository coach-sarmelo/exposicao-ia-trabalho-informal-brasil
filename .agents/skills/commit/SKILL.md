...
**Runs unconditionally.** Enforces that count claims (`"14 agents, 28 skills, 24 rules, 6 hooks"` and siblings) across README.md, AGENTS.md / GEMINI.md, the guide source + rendered HTML, the landing page, and the skill template all agree with the on-disk counts of `.agents/{skills,agents,rules,hooks}`:
...
Do NOT stage `.agents/settings.local.json` or any files containing secrets.
...
🤖 Generated with [Antigravity (agy)](https://claude.com/claude-code)
EOF
)"
```
