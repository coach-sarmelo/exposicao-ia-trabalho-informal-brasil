...
Surface the full permission-mode picture across every layer Antigravity (agy) honors, so the user can see at a glance why prompts are (or aren't) firing. Antigravity (agy) resolves permission mode from a 6-tier stack; a single misconfigured layer produces silent overrides that are hard to debug by eye.
...
3. **CLI user settings** — `~/.agents/settings.json`. Key: `permissions.defaultMode`.
4. **CLI project settings** — `<repo>/.agents/settings.json`. Same key. Wins over user.
5. **CLI project-local settings** — `<repo>/.agents/settings.local.json`. Same key. Wins over project.
...
Host-global settings files (`~/.agents/settings.json`, VSCode user settings) may contain:
...
CLI_PROJECT="${CLAUDE_PROJECT_DIR}/.agents/settings.json"
CLI_LOCAL="${CLAUDE_PROJECT_DIR}/.agents/settings.local.json"
```
...
> - `~/.agents/settings.json` (CLI user-level)
...
CLI_USER="${HOME}/.agents/settings.json"
```
...
The live in-session mode is exposed via the status line (see `.agents/plugins/academic-workflow/scripts/statusline.sh`). Tell the user:

> "Your status line shows the current in-session mode in the top-right of the Antigravity (agy) panel. If that disagrees with the resolved `defaultMode` above, you (or Shift+Tab) overrode it mid-session. Press Shift+Tab to cycle back."

If the status line isn't configured, emit a warning and point at `.agents/plugins/academic-workflow/scripts/statusline.sh`.
...
2. **VSCode-only bypass:** VSCode layers say bypass but no CLI layer does → terminal Antigravity (agy) will still prompt; extension may or may not.
3. **Empty allowlist + default mode:** `defaultMode: "default"` with empty `allow` → every tool prompts, as designed.
4. **Stale session:** settings are correct but user reports prompts → almost always a session that pre-dates the fix. Advise "Cmd+Shift+P → Developer: Reload Window, then new Antigravity (agy) session."
...
Check the status line at the top of the Antigravity (agy) panel. Expected: [BYPASS].
