...
Compare numeric claims in a manuscript (point estimates, standard errors, p-values, counts) against the actual outputs produced by the analysis pipeline. Report PASS / FAIL per claim against the tolerance thresholds defined in [`.agents/plugins/academic-workflow/rules/replication-protocol.md`](../../rules/replication-protocol.md).
...
**Tolerance source:** .agents/plugins/academic-workflow/rules/replication-protocol.md
...
See [`.agents/plugins/academic-workflow/rules/replication-protocol.md`](../../rules/replication-protocol.md) "Claims Provenance: `passport.yaml`" for the full schema and integration points (`/commit`, `/review-paper`).

## Cross-references

- [`.agents/plugins/academic-workflow/rules/replication-protocol.md`](../../rules/replication-protocol.md) — the tolerance contract + passport schema.
- [`templates/passport-template.yaml`](../../../templates/passport-template.yaml) — starter file to copy for a new paper.
- [`.agents/plugins/academic-workflow/skills/review-r/SKILL.md`](../review-r/SKILL.md) — catches code-style issues; this skill catches NUMERICAL reproducibility.
- [`.agents/plugins/academic-workflow/skills/diagnose/SKILL.md`](../diagnose/SKILL.md) — when a claim resolves to **FAIL** and you need to localize *which* pipeline step produced the out-of-tolerance value, hand off to `/diagnose` (single-claim root-cause: reproduce → minimise → bisect).
- [`.agents/plugins/academic-workflow/skills/review-paper/SKILL.md`](../review-paper/SKILL.md) — content review; pair with this skill for a full pre-submission audit.
- [`.agents/plugins/academic-workflow/skills/replication-package/SKILL.md`](../replication-package/SKILL.md) — gates on this skill before assembling the AEA DCAS deposit.
- [`.agents/plugins/academic-workflow/skills/capture-environment/SKILL.md`](../capture-environment/SKILL.md) · [`.agents/plugins/academic-workflow/skills/disclosure-check/SKILL.md`](../disclosure-check/SKILL.md) — environment capture + restricted-data screening downstream.
