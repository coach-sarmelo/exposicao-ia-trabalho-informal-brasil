...
- `personal-memory.md` and `.agents/state/` stay **local** (gitignored) — never expect a co-author to have yours (see meta-governance.md).
...
- [`.agents/plugins/academic-workflow/rules/meta-governance.md`](../../rules/meta-governance.md) — the cross-machine memory model (what syncs, what stays local) this brief operationalizes for *people*.
- [`.agents/plugins/academic-workflow/skills/capture-environment/SKILL.md`](../capture-environment/SKILL.md) — produces the lockfiles the "Reproduce locally" section points at.
- [`.agents/plugins/academic-workflow/skills/checkpoint/SKILL.md`](../checkpoint/SKILL.md) — the *self*-resume companion (this skill is the *other-person* handoff).
- [`.agents/plugins/academic-workflow/skills/compress-session/SKILL.md`](../compress-session/SKILL.md) — distil a noisy session before compaction (orthogonal; run before briefing if context is fat).
- [`.agents/plugins/academic-workflow/rules/confidential-data.md`](../../rules/confidential-data.md) — restricted-data handling; the data section summarizes its access steps without exposing data.
