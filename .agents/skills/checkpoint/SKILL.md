...
- For the narrative *what happened* — that lives in `quality_reports/session_logs/` (see `.agents/plugins/academic-workflow/rules/session-logging.md`).
...
- `.agents/plugins/academic-workflow/skills/checkpoint/SKILL.md:42` — body draft, needs trigger-phrase tightening
...
If the user says "yes" / "all" / "1 and 3" — append to MEMORY.md (root, the committed one) using the `[LEARN]` format. If the candidate is machine-specific (paths, tool versions, personal preference), recommend the user route it to `.agents/state/personal-memory.md` instead per `.agents/plugins/academic-workflow/rules/meta-governance.md`.
...
- `.agents/plugins/academic-workflow/rules/session-logging.md` — narrative companion. **Do not duplicate** — the checkpoint references the latest session log by path; it does not re-tell the session story.
- `.agents/plugins/academic-workflow/rules/plan-first-workflow.md` — checkpoint reads the active plan; if no plan exists, recommend the user enter plan mode before invoking `/checkpoint`.
- `templates/decision-record.md` — for *why we chose A over B*, not for *where we are*.
- `.agents/plugins/academic-workflow/hooks/pre-compact.py` — when `CLAUDE_PRECOMPACT_BLOCK_ON_DRAFT=1` is set, the PreCompact hook will block compaction once per DRAFT plan. `/checkpoint` is the right thing to run when that block fires.
