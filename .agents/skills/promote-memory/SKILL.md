...
<!-- Pattern adapted with attribution from Chris Blattman's claudeblattman v2.1
     ...
     .agents/plugins/academic-workflow/rules/meta-governance.md. -->
...
- **`.agents/state/personal-memory.md`** (gitignored, no size cap) — machine-specific and user-specific learnings.
...
- **For project-specific context.** That belongs in AGENTS.md / GEMINI.md or session logs, not in either memory tier.
...
> "Is this lesson already encoded in MEMORY.md, AGENTS.md / GEMINI.md, or an existing rule? Read the relevant files. If yes (even paraphrased), vote NO — duplication erodes the index's signal."
...
> "Does the entry follow the schema in [`.agents/plugins/academic-workflow/rules/meta-governance.md`](.agents/plugins/academic-workflow/rules/meta-governance.md): `[LEARN:category] wrong → right` for corrections, structured `**Why:**` + `**How to apply:**` for feedback/project entries? If it's just a free-form note, vote NO — fix the format first, then re-submit."
...
If `$ARGUMENTS` is `all`, read every `[LEARN:*]` entry in `.agents/state/personal-memory.md`. Otherwise treat `$ARGUMENTS` as a substring filter (e.g., `r-code` matches all `[LEARN:r-code]` entries).
...
- **Redundancy critic** — context: the candidate entry + the current `MEMORY.md` + `AGENTS.md / GEMINI.md` + relevant rule files.
- **Evidence critic** — context: the candidate entry only. Vote based on whether the entry self-describes its motivation.
- **Format critic** — context: the candidate entry + [`.agents/plugins/academic-workflow/rules/meta-governance.md`](.agents/plugins/academic-workflow/rules/meta-governance.md) for the schema reference.

Use the **Haiku tier** for all five critics (per [`.agents/plugins/academic-workflow/rules/model-routing.md`](../../rules/model-routing.md): mechanical-ish review work). The user can override via the agent's `model:` field if they want Sonnet for the harder calls.
...
- [`.agents/plugins/academic-workflow/rules/meta-governance.md`](../../rules/meta-governance.md) — the two-tier memory contract this skill operationalizes.
- [`.agents/plugins/academic-workflow/skills/promote-memory-council.md`](../../agents/promote-memory-council.md) — the five-critic implementation (one agent file with five role specs, dispatched in parallel via `Task`).
- [`.agents/plugins/academic-workflow/rules/model-routing.md`](../../rules/model-routing.md) — why critics default to Haiku tier.
