...
Focus: **all** executable code in the repo — `.agents/plugins/academic-workflow/hooks/*.py`, `.agents/plugins/academic-workflow/hooks/*.sh`, `scripts/*.py`, `scripts/*.sh`, `.agents/plugins/academic-workflow/scripts/*.sh`. Not just `.agents/plugins/academic-workflow/hooks/` — when PR #93 added new code under `scripts/`, the original narrow scope meant Copilot + Codex caught 5 bugs the audit missed.

Hook-specific checks (Stop/PreToolUse/SessionStart protocols, `CLAUDE_PROJECT_DIR` usage, hash-length consistency) apply only to `.agents/plugins/academic-workflow/hooks/`. Everything below applies to ALL executable code:

- No remaining `/tmp/` usage in anything that manages state (should use `~/.agents/sessions/`)
...
Focus: `.agents/plugins/academic-workflow/skills/*/SKILL.md` and `.agents/plugins/academic-workflow/rules/*.md`
...
- AGENTS.md / GEMINI.md skills table matches actual skill directories 1:1
- All templates referenced in `.agents/plugins/academic-workflow/rules/*.md` and the guide (`guide/workflow-guide.qmd`) exist in `templates/`
...
- `allowed-tools` linter warning — known linter bug (Antigravity (agy) issue #25380), field IS valid
...
- `log-reminder.py` outputting `{"decision": "block"}` with `sys.exit(0)` — this IS the modern Antigravity (agy) Stop-hook block protocol, NOT a bug
...
| State in /tmp/ | All Python hooks | Should use `~/.agents/sessions/<hash>/` |
