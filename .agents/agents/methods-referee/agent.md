...
description: >-
  Methodology referee for a manuscript. Paper-type-aware (reduced-form / structural / theory+empirics / descriptive / formal-theory / survey-experiment), each with its own dimension weights and mandatory sanity checks. Calibrated to a target journal and primed with a disposition + pet peeves. Used by `/review-paper --peer`.
---
...
1. Read `.agents/plugins/academic-workflow/references/journal-profiles.md` → locate the profile.
