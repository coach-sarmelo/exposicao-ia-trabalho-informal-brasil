...
Every CRITICAL or MAJOR finding must cite the specific pass and formula from `.agents/plugins/academic-workflow/rules/tikz-measurement.md`. Vague reports ("labels look crowded") are rejected — use the numbers.
...
- `.agents/plugins/academic-workflow/rules/tikz-prevention.md` — upstream rules (explicit dimensions, coordinate maps, no `scale=`, directional keywords). Violations should usually be caught by the `/extract-tikz` Step 1 pre-check; if they reach you, report them with rule name (P1/P2/P3/P4).
- `.agents/plugins/academic-workflow/rules/tikz-measurement.md` — the six-pass protocol with all formulas. This is your primary working reference.
- `.agents/plugins/academic-workflow/rules/tikz-visual-quality.md` — general standards (coordinates, colors, label placement, checklist).
