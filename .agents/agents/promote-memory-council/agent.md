...
description: >-
  Five-critic council that votes YES/NO on whether candidate `[LEARN]` entries should be promoted from `.agents/state/personal-memory.md` (gitignored) to MEMORY.md (committed). Each critic reviews one dimension — generality, staleness, redundancy, evidence, format — in an isolated forked context. Invoked by `/promote-memory`.
---
...
**Your question:** Is this lesson already encoded in MEMORY.md, AGENTS.md / GEMINI.md, or an existing rule?

**Vote NO** when:
- The same lesson appears in MEMORY.md (even paraphrased).
- The lesson is codified in a rule file (`.agents/plugins/academic-workflow/rules/*.md`).
- AGENTS.md / GEMINI.md already pins the behaviour as a project principle.
...
**Your question:** Does the entry follow the schema in [`.agents/plugins/academic-workflow/rules/meta-governance.md`](../rules/meta-governance.md)?
