...
Symmetric partner to **Pre-Flight Reports** (`.agents/plugins/academic-workflow/rules/content-invariants.md` + skill-level `## Phase 0`). Where Pre-Flight proves inputs were read *before* work, Post-Flight proves the output's factual claims hold *after* drafting — before the skill returns to the user.
...
- `.agents/agents/claim-verifier.md` — the forked verifier.
- `.agents/plugins/academic-workflow/skills/verify-claims/SKILL.md` — user-facing wrapper for ad-hoc verification of any text.
- `.agents/plugins/academic-workflow/rules/content-invariants.md` — Pre-Flight (input side).
- `.agents/plugins/academic-workflow/rules/cross-artifact-review.md` — pattern-based; Post-Flight is draft-based.
- `.agents/plugins/academic-workflow/rules/summary-parity.md` — rule against enumerative summaries drifting from their bodies; Post-Flight is the factual equivalent for draft content.
