...
- **`MEMORY.md` syncs via git; `.agents/state/personal-memory.md` stays local** (`meta-governance.md`). Machine-specific restricted-data *paths* live in personal-memory, never in committed files.
...
- [`.agents/plugins/academic-workflow/rules/replication-protocol.md`](replication-protocol.md) — passport entries note disclosure status.
- [`.agents/plugins/academic-workflow/rules/meta-governance.md`](meta-governance.md) — the committed-vs-local two-tier model this builds on.
