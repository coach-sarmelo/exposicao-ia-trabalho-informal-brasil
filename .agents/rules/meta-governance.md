...
### .agents/state/personal-memory.md (gitignored, local only)
...
- Builds `.agents/state/personal-memory.md` specific to desktop setup

**Machine B (laptop):**
- Clone same repo → gets same MEMORY.md ✓
- Gets same infrastructure ✓
- Builds DIFFERENT `.agents/state/personal-memory.md` for laptop setup
...
Configure in AGENTS.md / GEMINI.md for your setup.
```
...
```markdown
Configure bibliography location in AGENTS.md / GEMINI.md:
...
### AGENTS.md / GEMINI.md Can Have Placeholders

The template AGENTS.md / GEMINI.md has `[YOUR PROJECT NAME]`, `[YOUR INSTITUTION]` — this is correct. Users fill them in.
...
- Cross-machine workflows change (e.g., Antigravity (agy) adds cloud sync)
...
| Machine-specific setup | ❌ No | .agents/state/personal-memory.md | ❌ No (gitignored) |
| Templates (generic) | ✅ Yes | templates/ | ✅ Yes |
| Skills (generic) | ✅ Yes | .agents/plugins/academic-workflow/skills/ | ✅ Yes |
| Rules (path-scoped, generic) | ✅ Yes | .agents/plugins/academic-workflow/rules/ | ✅ Yes |
| Agents (generic) | ✅ Yes | .agents/agents/ | ✅ Yes |
| Hooks (generic behavior) | ✅ Yes | .agents/hooks/ | ✅ Yes |
| Session logs | ✅ Yes | quality_reports/session_logs/ | ✅ Yes |
| Plans | ✅ Yes | quality_reports/plans/ | ✅ Yes |
| Local settings | ❌ No | .agents/settings.local.json | ❌ No (gitignored) |
| Session state | ❌ No | .agents/state/ | ❌ No (gitignored) |
...
- Keep specific learnings local (.agents/state/personal-memory.md, gitignored)
