...
| "Edits to `.agents/agents/X.md` and `.agents/plugins/academic-workflow/skills/Y/SKILL.md`" | "Existing infrastructure revised" |
...
- `.agents/plugins/academic-workflow/skills/commit/SKILL.md` — when writing the commit message for a doc-heavy PR, apply this rule to the `## Summary` section before pushing.
- `.agents/plugins/academic-workflow/skills/audit-reproducibility/SKILL.md` — "Repeated EXPLAINED is a signal (two-strikes)" reuses this pattern for contested numeric claims.
