...
**How to use.** When writing a deep-audit agent prompt, say "read `.agents/plugins/academic-workflow/references/audit-pet-peeves.md` and explicitly check for each class before reporting clean." When triaging a Copilot/Codex finding, decide: is this a new class (add entry) or an existing class re-surfacing (bump the evidence list)?
...
**Example:** PR #87. `.agents/hooks/log-reminder.py` docstring said "blocks Claude from stopping" but the code had been rewritten to stderr-only advisories. Copilot caught.
...
**How to catch.** Not mechanical — it's a judgment pattern. The rule `.agents/plugins/academic-workflow/rules/summary-parity.md` encodes it: two review-bot flags on the same summary paragraph = rewrite structurally (abstract up), don't patch.
...
**Why deep-audit missed it.** Agent 2's original scope was `.agents/hooks/` only. New code in `scripts/` bypassed the audit entirely. Even with correct scope, "docstring claims X but code does Y" is a class that requires reading both carefully — easy to miss on a skim.
...
**Example:** 2026-04-16. The user's usage was about to be rate-limited and they needed autonomous work to fire ~1 hour later. We used `CronCreate` with `durable: true`, but the cron is tied to the Antigravity (agy) REPL's own event loop — when the session died (rate limit + user walked away + VSCode closed the panel), the cron went with it. Plan survived on disk; the scheduled trigger did not. User had to invoke manually on return.

**How to catch.** Any time you use `CronCreate` with a delay > 10 minutes, ask: "will the Antigravity (agy) session definitely be running at fire time?" If not, switch to **Antigravity (agy) Routines** (released Apr 14, 2026) — they run on Anthropic's web infrastructure and don't depend on the local REPL. Routines are the right primitive for overnight work, scheduled audits, and autonomous runs while the user is away. `CronCreate` is for short-delay polling within an active session.
...
- `AGENTS.md / GEMINI.md` "Skills Quick Reference" table (rows = N).
- `README.md` skills/agents tables.
- `docs/index.html` inline bullet lists that enumerate skills.
A future mechanical check could count `.agents/{skills,agents,rules}/*` and grep table rows in these surfaces; until then, deep-audit Agent 1 + Agent 4 should explicitly check appendix table row counts.
...
**When to apply.** Whenever a release adds or removes a skill/agent/rule/hook. Surface-sync clean ≠ enumerative tables current. Run a quick `wc -l` style check of appendix tables vs `ls .agents/plugins/academic-workflow/skills/ | wc -l`.
...
- Reference this file from `.agents/plugins/academic-workflow/skills/deep-audit/SKILL.md` so all 4 agents load it.
