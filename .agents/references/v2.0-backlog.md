...
- ✅ **Stata expansion** — `/stata-replication` + `.agents/plugins/academic-workflow/rules/stata-code-conventions.md` + `/audit-reproducibility` Stata coverage shipped (Pass 4). Was not originally on the v1.9 backlog but added per user request mid-cycle.
...
The v2.0.0 "modernization" release (2026-06-09) shipped the loop-first / gate-enforced paradigm shift plus the long-deferred capability skills. See [CHANGELOG.md v2.0.0](../../CHANGELOG.md).
...
- ✅ **Real orchestration runtime** — `orchestrator-protocol.md` rewritten from "pattern, not a runtime" into fan-out → reduce → judge + hallucination gate → loop-until-dry (`orchestration-schemas.md`, `agent-fleet.md`).
...
- **Non-Claude coauthor gate note** — document the path for a coauthor who pulls the repo *without* Antigravity (agy): `./scripts/install-hooks.sh` works for them too (pre-commit is plain bash/python), but skills/hooks guidance assumes the Claude loop; a short "for your non-Claude coauthor" README box would close it.
...
Considered for v1.10.0 and pulled before ship: a `.claude-plugin/marketplace.json` exposing `review-suite` / `simulation-suite` as installable subsets. The `strict:false` in-place schema has known path-resolution quirks ([#11278](https://github.com/anthropics/claude-code/issues/11278)) and needs interactive validation. **Re-affirmed as a documented non-goal in v2.0** (see CHANGELOG v2.0.0 "Notes"); the template stays fork-the-whole-thing until #11278 resolves.
