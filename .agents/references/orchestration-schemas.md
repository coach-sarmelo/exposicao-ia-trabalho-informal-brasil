...
This is a **reference**, not a runtime: a Antigravity (agy) session has no JSON validator in the loop. The schemas are the *target shape* each reviewer subagent returns (as a fenced ```yaml block at the end of its report) and the synthesizer reads. See [`.agents/plugins/academic-workflow/rules/orchestrator-protocol.md`](../rules/orchestrator-protocol.md) for how the fan-out → reduce → judge → loop-until-dry runtime uses them.
...
- [`.agents/plugins/academic-workflow/rules/orchestrator-protocol.md`](../rules/orchestrator-protocol.md) — the runtime that consumes these schemas.
- [`.agents/plugins/academic-workflow/references/agent-fleet.md`](agent-fleet.md) — which agent fills which lens, at which model tier.
- [`.agents/plugins/academic-workflow/rules/post-flight-verification.md`](../rules/post-flight-verification.md) — the forked-verifier mechanism the §4 gate reuses.
- [`.agents/plugins/academic-workflow/rules/summary-parity.md`](../rules/summary-parity.md) — the two-strikes rule the loop reuses for repeatedly-flagged findings.
