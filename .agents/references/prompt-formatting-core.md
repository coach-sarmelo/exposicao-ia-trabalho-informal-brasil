...
One sentence naming the role the agent should adopt. Specific over generic.
...
The bookend is what catches the cases where the prompt was almost-but-not-quite specific enough. Without it, the agent guesses; with it, you get one focused question that improves the rest of the run.
...
- What you emit: All six sections + Assumptions block + an **Investigation pre-step** before the main task. The pre-step is one sentence describing what the agent should research / read / verify before producing the output. Example: "Pre-step: read the Methods section of [paper] and identify the assumption set actually invoked before assessing whether the paper's main claim follows."
